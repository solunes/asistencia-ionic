  ...
  getData(){
    this.storage.get(this.key_page).then(data => {
        ...
        if (data) {
          ...
          last_id = this.http.getLastId(this.notifications)
        }
        if (!this.loading.loading_page) {
          this.loading.showLoading(last_id)
          this.http.getRequest(this.key_page, last_id).subscribe((result: Array<JSON>) => {
            for (var i = 0; i < result['notifications'].length; i++) {
              this.notifications.unshift(result['notifications'][i])
            }
            this.storage.set(this.key_page, this.notifications);
